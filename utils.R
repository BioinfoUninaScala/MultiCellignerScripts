...
#'
#' Impute missing values separately within lineage-specific sample groups
#' 
#' @name imputeNA_by_lineage
#'
#' @param mat matrix of samples by features containing missing values.
#' @param annot annotation matrix containing lineage information for each sample.
#' @param sample_column column in annot matching the rownames of mat.
#' @param th optional threshold for filtering features with excessive missing values. Default is 0.8.
#'
#' @description Split samples by lineage and perform KNN-based imputation independently for each lineage subset using common features across groups.
#'
#' @return Matrix with imputed values for all lineage-specific subsets.
#' @export
#'

imputeNA_by_lineage <- function(mat, annot, sample_column, th = 0.8){
  
  message('Removing genes with many missing values...')
  lineages <- annot %>% pull(lineage) %>% unique()
  clean_mat_list <-
    lineages %>% lapply(., function(lin){
      print(lin)
      lineage_samples <- annot %>% filter(lineage == lin) %>% pull(sample_column)
      lineage_sub_mat <- mat[rownames(mat) %in% lineage_samples,]
      lineage_sub_mat_clean <- lineage_sub_mat #[,(colSums(is.na(lineage_sub_mat)) < (th*nrow(lineage_sub_mat)))]
      lineage_sub_mat_clean
    })
  
  message('Subset matrices for common genes...')
  common_cols <- Reduce(intersect, lapply(clean_mat_list, colnames))
  lineage_sub_cmgenes <- lapply(clean_mat_list, function(mat) {
    mat_cmgenes <- mat[, common_cols, drop = FALSE]
    message('Imputing missing values...')
    mat_cmgenes_impute <- impute.knn(t(mat_cmgenes))
    mat_cmgenes_impute$data
  })
  
  lineage_imputed_mat <- do.call(cbind, lineage_sub_cmgenes)
  return(lineage_imputed_mat)
}

#'
#' Parallel fitting of mutational signatures across all samples
#' 
#' @name get_fitting_signature_p
#'
#' @import sigminer
#'
#' @param nmf_mat SBS-96 mutational count matrix with samples in rows.
#' @param cores number of cores used for parallel computation.
#'
#' @description Perform parallel fitting of COSMIC mutational signatures for each sample using sigminer and return relative signature exposures.
#'
#' @return Matrix of relative mutational signature exposures for all samples.
#' @export
#'

get_fitting_signature_p <- function(nmf_mat, cores) {
...
#' THE FOLLOWING FUNCTIONS HAVE BEEN TAKEN FOR CELLIGNER: https://github.com/broadinstitute/celligner
#' 
